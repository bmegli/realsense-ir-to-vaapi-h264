...
int main_loop(const input_args &input, rs2::pipeline &pipe, AVCodecContext *avctx, ofstream &out_file)
{
    ...
    for(int f=0;f<frames;++f)
	{
		...
		cout << (f+1)  << ": width " << ir_frame.get_width() << " height " << ir_frame.get_height() << " stride="
		<< ir_frame.get_stride_in_bytes() << " bytes " << ir_frame.get_stride_in_bytes() * ir_frame.get_height() << endl;
...
bool init_av(struct av_args *av, const input_args &input)
{
	...
	if( (err = av_hwdevice_ctx_create(&av->hw_device_ctx, AV_HWDEVICE_TYPE_VAAPI,
...
{	
    if (argc < 5)
	{
        cerr << "Usage: " << argv[0] << " <width> <height> <framerate> <seconds>" << endl;
